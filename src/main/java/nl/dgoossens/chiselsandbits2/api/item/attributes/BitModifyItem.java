 ...
 */
public interface BitModifyItem {
    /**
    ...
    static enum ModificationType {
        ...
        CUSTOM, //Unknown type of action, use an interface extending BitModifyItem and specify it
        ;
        ...
        private boolean validateUsedButton(boolean leftClick, ItemStack item) {
            switch (this) {
                ...
                case ROTATE:
                    return ItemPropertyUtil.isItemMode(item, ItemModes.WRENCH_ROTATE, ItemModes.WRENCH_ROTATECCW) && !leftClick; //Right Click
                case MIRROR:
                    return ItemPropertyUtil.isItemMode(item, ItemModes.WRENCH_MIRROR) && !leftClick; //Right Click
            }
            return false;
        }
    }
