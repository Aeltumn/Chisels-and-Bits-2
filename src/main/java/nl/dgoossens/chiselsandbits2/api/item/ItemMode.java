...
public interface ItemMode {
     ...
     * Only applicable if the type is {@link ItemModeTypes#CHISEL}!
     *
     ...
     */
    public default ChiselIterator getIterator(final BlockPos bitPosition, final Direction side, final BitOperation place, @Nullable final VoxelTile source) {
        return getIterator(bitPosition, side, place, source, null, null);
    }

    /**
     * Variant of the normal {@link #getIterator(BlockPos, Direction, BitOperation, VoxelTile)} where an entire
     * region can be passed to look at.
     *
     * @param from Start of the iterator's area
     * @param to   End of the iterator's area
     */
    public default ChiselIterator getIterator(final BlockPos bitPosition, final Direction side, final BitOperation place, @Nullable final VoxelTile source, @Nullable final BitLocation from, @Nullable final BitLocation to) {
        if (getType().equals(ItemModeTypes.CHISEL)) throw new UnsupportedOperationException("Mode " + toString() + " doesn't have an implementation for its iterator!");
