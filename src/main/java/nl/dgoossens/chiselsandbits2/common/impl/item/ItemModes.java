...
import nl.dgoossens.chiselsandbits2.common.impl.iterators.ChiselTypeIterator;

/**
 * The current mode the item is using shared between all item mode types in base C&B2.
 */
public enum ItemModes implements ItemModeEnum {
    CHISEL_SINGLE(3, 3),
    CHISEL_LINE(6, 6),
    CHISEL_PLANE(9, 9),
    ...
    @Override
    public ChiselIterator getIterator(BlockPos bitPosition, Direction side, BitOperation place, VoxelTile source) {
        /*if (this == ItemModes.CHISEL_CONNECTED_MATERIAL)
            return new ChiselExtrudeMaterialIterator(bitPosition, source, side, place.equals(PLACE));

        if (this == ItemModes.CHISEL_CONNECTED_PLANE)
            return new ChiselExtrudeIterator(bitPosition, source, side, place.equals(PLACE));

        if (this == ItemModes.CHISEL_SAME_MATERIAL)
    ...
    @Override
    public ChiselIterator getIterator(final BlockPos bitPosition, final Direction side, final BitOperation place, final VoxelTile source, final BitLocation from, final BitLocation to) {
        if (this == ItemModes.CHISEL_DRAWN_REGION) {
